...
export default function Order_Operator() {
  ...
  useEffect(() => {
    ...
    async function getInvoice() {
      try {
        const data = await axios.get(`${process.env.NEXT_PUBLIC_URL}/api/invoice?status=sample dikerjakan operator${year ? `&year=${year}` : ''}${month ? `&month=${month}` : ''}${jenis_pengujian ? `&jenis_pengujian=${jenis_pengujian}` : ''}&skip=${page * 15}&limit=15`, { withCredentials: true })
  ...
  return (
    <>
      ...
      <div className='flex flex-wrap justify-center mb-10'>
        ...
        <div className='md:flex grid grid-cols-2 p-1 mt-2  justify-between grad rounded-lg md:ml-3 sm:ml-3 md:w-72 sm:w-64 w-52  ml-3'><p className="md:text-lg sm:text-base text-xs font-semibold text-white p-2">Bulan :</p> <select className='ml-3' name="bulan" id="bulan" onChange={(e) => setMonth(e.target.value)}>
          <option value="" defaultChecked className='input-style-lki'>all</option>
          {monthOption.map((v, i) => {
