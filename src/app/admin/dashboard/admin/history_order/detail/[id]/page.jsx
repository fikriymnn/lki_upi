...
export default function HAdetail({ params, searchParams }) {
  ...
  const downloadKuitansi = async (e) => {
    try {
      ...
      const link = document.createElement('a');
      link.href = window.URL.createObjectURL(blob);
      link.download = `kuitansi_${invoice?.id_user?.nama_lengkap.replace(" ","_")}_${new Date().getDate()}_${new Date().getMonth() + 1}_${new Date().getFullYear()}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (err) {
    ...
    <>
      <div>
        ...
        <div className="grid md:grid-cols-2 sm:grid-cols-2 grid-cols-1 gap-2 md:mx-20 mx-5">
          <p className="text-xs border-2 rounded-lg p-2 border-b-2 grid grid-cols-2 md:text-xl sm:text-xl  font-semibold">status : <span className="ml-3 font-semibold text-gray-600 md:text-base sm:text-sm text-xs my-auto ">{invoice?.status}</span> </p>
          {invoice?.status == "form dikonfirmasi" ? <p>*kirim sample ke alamat yang tertera \n (Jl.lorem ipsum dolor)</p> : ""}
          <p className="text-xs border-2 rounded-lg p-2 border-b-2 grid grid-cols-2 md:text-xl sm:text-xl  font-semibold">total harga  : <span className="ml-3 font-semibold text-gray-600 md:text-base sm:text-sm text-xs my-auto "> Rp.{invoice?.total_harga}</span></p>
