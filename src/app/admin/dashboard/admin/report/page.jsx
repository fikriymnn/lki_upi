  ...
  useEffect(() => {
    ...
    async function getInvoice() {
      try {
        ...
        setLoading(true)
        const data = await axios.get(`${process.env.NEXT_PUBLIC_URL}/api/order?report=true&status_pengujian=success&skip=${page * 100}&limit=100${month ? `&month=${month}` : ""}${year ? `&year=${year}` : ""}${jenis_pengujian ? `&jenis_pengujian=${jenis_pengujian}` : ""}`, { withCredentials: true })
