...
export default function AffiliateLayananAnalisis({ affiliateId }) {
  ...
  }, [affiliateId, search, statusFilter]);
  ...
  const openEdit = (item) => {
    setMode('edit');
    setForm({ id: item._id, id_affiliate: affiliateId, nama_layanan: item.nama_layanan, is_active: item.is_active });
    setShowModal(true);
  };
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-1 flex items-center gap-1">
            <Building2 className="w-3 h-3" /> {affiliateName}
          </p>
        </div>
        <button
          onClick={openCreate}
