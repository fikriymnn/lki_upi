...
import {
  ...
  FileText, FileCheck, Download, Eye, Receipt, CheckCircle2, XCircle, PlayCircle,
  UploadCloud, User, Loader2, Pencil, Save, X, Plus, Trash2, ChevronDown, FileSpreadsheet
} from 'lucide-react';
...
// ── Laboran bisa bertindak saat order di 3 tahap ini: konfirmasi → mulai proses → upload laporan ──
const LABORAN_ACTIONABLE_STATUS = ['Menunggu Order Dikonfirmasi', 'Order Dikonfirmasi', 'Order Diproses'];
...
// Kalau berupa nama file saja, dibangun jadi {FILE_URL}/file/{category}/{filename}.
...
// ASUMSI kontrak response backend: { data: { filename } } atau { filename } atau { downloadURL }.
// ASUMSI nama field form-data: "file". Sesuaikan kalau kontrak API kamu beda.
...
export default function AffiliateOrder({ affiliateId }) {
  ...
  // ── Form upload laporan & rincian biaya (khusus tahap "Order Diproses") — sekarang file asli ──
  const [formLaporan, setFormLaporan] = useState({ laporan: '', rincian_biaya: '' });
  const [savingLaporan, setSavingLaporan] = useState(false);
  const [uploadState, setUploadState] = useState({
    laporan: { uploading: false, progress: 0 },
    rincian_biaya: { uploading: false, progress: 0 },
  });
  ...
  const openDetail = async (order) => {
    ...
    setFormLaporan({ laporan: order.laporan || '', rincian_biaya: order.rincian_biaya || '' });
    ...
    try {
      ...
      setActiveOrder(full);
      setFormLaporan({ laporan: full.laporan || '', rincian_biaya: full.rincian_biaya || '' });
    } catch (err) {
  ...
  // ─────────────────────────── Aksi ubah status (konfirmasi/tolak/mulai proses) ───────────────────────────
  ...
  const handleKonfirmasi = () => changeStatus('Order Dikonfirmasi');
  const handleTolak = () => {
    if (!confirm('Tolak order ini?')) return;
    changeStatus('Order Ditolak');
  };
  const handleMulaiProses = () => changeStatus('Order Diproses');

  // ─────────────────────────── Upload file laporan / rincian biaya (langsung ke API file) ───────────────────────────
  const handleFileSelect = async (key, file) => {
    if (!file) return;
    setUploadState((s) => ({ ...s, [key]: { uploading: true, progress: 0 } }));
    try {
      const url = await uploadFileToServer(file, FILE_CATEGORY[key], (percent) => {
        setUploadState((s) => ({ ...s, [key]: { uploading: true, progress: percent } }));
      });
      setFormLaporan((f) => ({ ...f, [key]: url }));
    } catch (err) {
      alert(err.response?.data?.message || err.message || 'Gagal upload file');
    } finally {
      setUploadState((s) => ({ ...s, [key]: { uploading: false, progress: 0 } }));
    }
  };

  // ─────────────────────────── Simpan Laporan & Rincian Biaya (URL hasil upload) ───────────────────────────
  const handleSaveLaporan = async () => {
    if (savingLaporan) return;
    try {
      setSavingLaporan(true);
      const res = await axios.put(
        `${process.env.NEXT_PUBLIC_URL}/api/order_affiliate/${activeOrder._id}/laporan`,
        formLaporan,
        { withCredentials: true }
      );
      if (res.data.success) {
  ...
  // Dipakai ItemSectionEditable untuk upload foto_sample / jurnal_pendukung
  const uploadItemFile = (key, file, onProgress) => uploadFileToServer(file, FILE_CATEGORY[key], onProgress);

  // ─────────────────────────── Download Invoice PDF (view-only) ───────────────────────────
  ...
  const actionLabel = (order) =>
    LABORAN_ACTIONABLE_STATUS.includes(order.status_pengujian) ? 'Proses' : 'Detail';
      ...
      <div className="p-6 max-w-5xl mx-auto">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-gray-500">Detail order — konfirmasi, proses sampel, dan upload laporan</p>
          </div>
        </div>
        ...
        {/* ── Panel aksi: Konfirmasi / Tolak — saat "Menunggu Order Dikonfirmasi" ── */}
        {activeOrder.status_pengujian === 'Menunggu Order Dikonfirmasi' && (
          <div className="border border-blue-200 bg-blue-50 rounded-xl p-4 mb-6">
            <p className="text-xs font-semibold text-blue-800 mb-1 flex items-center gap-1.5">
              <ClipboardList className="w-3.5 h-3.5" /> Konfirmasi Order
            </p>
            <p className="text-[11px] text-blue-700 mb-3">
              Periksa data pemohon & layanan yang diminta di tab Rincian Order. Konfirmasi untuk mulai memproses, atau tolak jika tidak bisa dilayani.
            </p>
            <div className="flex flex-wrap gap-2">
              ...
              >
                <XCircle className="w-3.5 h-3.5" /> Tolak Order
              </button>
            </div>
          </div>
        )}

        {/* ── Panel aksi: Mulai Proses — saat "Order Dikonfirmasi" ── */}
        {activeOrder.status_pengujian === 'Order Dikonfirmasi' && (
          <div className="border border-indigo-200 bg-indigo-50 rounded-xl p-4 mb-6">
            <p className="text-xs font-semibold text-indigo-800 mb-1 flex items-center gap-1.5">
              <PlayCircle className="w-3.5 h-3.5" /> Mulai Proses Sample
            </p>
            <p className="text-[11px] text-indigo-700 mb-3">
              Order sudah dikonfirmasi. Mulai proses sample untuk lanjut ke tahap upload laporan & rincian biaya.
            </p>
            <button
        ...
        {/* ── Panel aksi: Upload Laporan & Rincian Biaya — saat "Order Diproses" (upload file langsung) ── */}
        {activeOrder.status_pengujian === 'Order Diproses' && (
          <div className="border border-yellow-200 bg-yellow-50 rounded-xl p-4 mb-6">
            ...
            <div className="flex flex-col gap-3">
              <FileUploadField
                label="Laporan"
                fieldKey="laporan"
                value={formLaporan.laporan}
                uploadState={uploadState.laporan}
                onSelect={(file) => handleFileSelect('laporan', file)}
              />
              <FileUploadField
                label="Rincian Biaya"
                fieldKey="rincian_biaya"
                value={formLaporan.rincian_biaya}
                uploadState={uploadState.rincian_biaya}
                onSelect={(file) => handleFileSelect('rincian_biaya', file)}
              />
              <button
                onClick={handleSaveLaporan}
                disabled={
                  savingLaporan ||
                  uploadState.laporan.uploading ||
                  uploadState.rincian_biaya.uploading ||
                  (!formLaporan.laporan?.trim() && !formLaporan.rincian_biaya?.trim())
                }
                className="self-start flex items-center gap-1.5 px-4 py-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-xs font-medium rounded-lg transition"
              >
                {savingLaporan ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <UploadCloud className="w-3.5 h-3.5" />}
                Simpan
              </button>
            </div>
          </div>
        ...
        {/* ── Info read-only — status di luar kendali laboran ── */}
        {!LABORAN_ACTIONABLE_STATUS.includes(activeOrder.status_pengujian) && (
          <div className="border border-gray-200 bg-gray-50 rounded-xl p-4 text-center mb-6">
            <p className="text-xs text-gray-500">
              Order ini sedang ditangani oleh {statusOwnerHint(activeOrder.status_pengujian)}. Sebagai laboran, Anda hanya bisa bertindak pada tahap konfirmasi, mulai proses, dan upload laporan — namun tetap bisa mengedit data order di tab Rincian Order.
            </p>
          </div>
        )}
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6 ml-12">
        Konfirmasi order, proses sampel, & upload laporan hasil pengujian
      </p>
          ...
          <table className="w-full min-w-[820px] border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                const needsAction = LABORAN_ACTIONABLE_STATUS.includes(v.status_pengujian);
                return (
                  <tr key={v._id} className="hover:bg-gray-50/80 transition">
                    ...
                    <td className="px-4 py-4 align-middle text-right">
                      ...
                      >
                        {needsAction ? <ClipboardList className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                        {actionLabel(v)}
                      </button>
                    </td>
                  </tr>
...
function statusOwnerHint(status) {
  switch (status) {
    case 'Menunggu Diverifikasi': return 'ketua lab (verifikasi laporan)';
    case 'Order Ditolak': return 'tidak ada — order sudah ditolak';
...
// ── Input file dengan progress bar, dipakai di panel Upload Laporan & Rincian Biaya ──
function FileUploadField({ label, value, uploadState, onSelect }) {
  const inputId = `upload-${label.replace(/\s+/g, '-').toLowerCase()}`;
  return (
    <div>
      <label className="text-[11px] font-medium text-yellow-800">{label}</label>
      <div className="mt-1 flex items-center gap-2 flex-wrap">
        ...
        <label
          htmlFor={inputId}
          className="cursor-pointer flex items-center gap-1.5 px-3 py-2 border border-yellow-300 bg-white rounded-lg text-xs font-medium text-yellow-800 hover:bg-yellow-50 transition"
        >
          <UploadCloud className="w-3.5 h-3.5" /> {value ? 'Ganti File' : 'Pilih File'}
        </label>
        {uploadState.uploading && (
          <span className="text-[11px] text-yellow-700">Mengunggah... {uploadState.progress}%</span>
        )}
        {value && !uploadState.uploading && (
          <a href={value} target="_blank" rel="noopener noreferrer" className="text-[11px] text-red-700 underline">
            Lihat file saat ini
          </a>
        )}
      </div>
