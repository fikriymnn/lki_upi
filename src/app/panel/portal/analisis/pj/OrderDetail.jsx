...
export default function OrderDetail({ setActivePage, idInvoice, noInvoice }) {
  ...
  const handleConfirmVerif = async (e) => {
    e.preventDefault();
    setEditVerif(false);
    let obj = { status: formVerif.status, catatan: formVerif.catatan };
    try {
      function timeNow() {
        const d = new Date();
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Informasi dan verifikasi order pengujian</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="space-y-4 divide-y divide-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-3 items-start pt-4 first:pt-0">
              <p className="text-sm font-medium text-gray-600 pt-1.5">Status Verifikasi</p>
              {editVerif ? (
                <select
                  name="status"
                  value={formVerif.status}
                  onChange={(e) => setFormVerif((a) => ({ ...a, [e.target.name]: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-transparent"
                >
                  <option value="Menunggu Verifikasi">Menunggu Verifikasi</option>
                  <option value="Menunggu Pembayaran">Verifikasi Selesai (Menunggu Pembayaran)</option>
                </select>
              ) : (
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold w-fit ${statusBadge(formVerif.status)}`}>
                  {formVerif.status || "—"}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-3 items-start pt-4">
              ...
              {editVerif ? (
                ...
                <p className="text-sm text-gray-700 pt-1.5">{formVerif.catatan || "—"}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 mt-6 pt-5 border-t border-gray-200">
            {editVerif ? (
              <>
                ...
                >
                  <Check className="w-4 h-4" /> Konfirmasi Verifikasi
                </button>
              </>
            ) : (
              <button
