...
export default function OrderDetail({ setActivePage, idInvoice, noInvoice }) {
  ...
  // ── Tabs config — 2 tab saja, Dokumen dihapus ────────────
  const tabs = [
    { key: "info",   label: "Informasi Pelanggan", icon: <User  className="w-4 h-4" /> },
    { key: "status", label: "Update Progress",     icon: <Edit2 className="w-4 h-4" /> }, // ← icon pensil, label baru
  ];
  ...
  // ── Handler konfirmasi (tidak diubah) ────────────────────
