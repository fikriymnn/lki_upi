    ...
    async function getInvoice() {
      ...
      try {
        const data = await axios.get(
          `${process.env.NEXT_PUBLIC_URL}/api/invoice?status=Sample Dikerjakan Operator${year ? `&year=${year}` : ""}${month ? `&month=${month}` : ""}${jenis_pengujian ? `&jenis_pengujian=${jenis_pengujian}` : ""}&skip=${page * 15}&limit=15`,
                  ...
                  <tr key={i} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {/* Detail — nanti pakai setActivePage */}
                        <button
                          onClick={() => {
                        ...
                        {/* Konfirmasi/Tracking — nanti pakai setActivePage */}
                        <button
                          onClick={() => {
                            setNoInvoice(v.no_invoice);
                            setIdInvoice(v._id);
